...
# Email professor the project zipped so that he can take a look at backtesting
# Use T test to confirm drift
...
# This shows a possible AR(1) behavior, but most likely unit root
eacf(chevron.train)
...
bt0 <- backtest(m0, chevron.train, orig = chevron.test, h = 1)
...
# arch model produces very significant coefficients
arch.fit <- garch(res, order = c(0, 1))
arch.fit
coeftest(arch.fit)

# Residuals look less volatile, but there is still some volatility present
autoplot(arch.fit$residuals)
Acf(arch.fit$residuals)

# Ljung box test definitely rejects the null hypothesis
Box.test(arch.fit$residuals, type = 'Ljung')
...
autoplot(garch.fit$residuals)
Acf(garch.fit$residuals)
...
# "Error in if (orig > T) orig = T : the condition has length > 1"
bt1 <- backtest(m1, chevron.train, orig = chevron.test, h = 1)
...
# "Error in if (orig > T) orig = T : the condition has length > 1"
bt3 <- backtest(m3, chevron.train, orig = chevron.test, h = 1)
...
# Ljung box test still rejects the null hypothesis, but much closer to failing
# to reject it
Box.test(arch.fit$residuals, type = 'Ljung')
...
# "Error in if (orig > T) orig = T : the condition has length > 1"
bt4 <- backtest(m4, chevron.train, orig = chevron.test, h = 1)
...
# Ljung box test still rejects the null hypothesis
Box.test(arch.fit$residuals, type = 'Ljung')
...
# "Error in if (orig > T) orig = T : the condition has length > 1"
bt5 <- backtest(m5, chevron.train, orig = chevron.test, h = 1)
...
# "Error in if (orig > T) orig = T : the condition has length > 1"
bt6 <- backtest(m6, chevron.train, orig = chevron.test, h = 1)
